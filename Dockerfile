...
RUN wget https://sonatype-download.global.ssl.fastly.net/nexus/3/nexus-3.2.0-01-unix.tar.gz -O /tmp/nexus-3.2.0-01-unix.tar.gz && \
        ...
        tar -C /opt/sonatype/ -xvaf /tmp/nexus-3.2.0-01-unix.tar.gz && \
        ln -s /opt/sonatype/nexus-3.2.0-01/ /opt/sonatype/nexus && \
        rm -f /tmp/nexus-3.2.0-01-unix.tar.gz && \
        chown -Rv nexus:nexus /opt/sonatype/nexus && \
        chown -Rv nexus:nexus /opt/sonatype/nexus-3.2.0-01 && \
